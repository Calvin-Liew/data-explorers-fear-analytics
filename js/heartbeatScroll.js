(function () {
  ...
  function buildSegmentPolylineH(y0, x0, x1, preset) {
    ...
    function phaseOffset(ph) {
      ...
      if (ph < 0.14) {
        ...
        yOffset += A * Aq * s(t);
      } else if (ph >= 0.26 && ph < 0.3) {
        const t = (ph - 0.26) / 0.04;
        yOffset -= A * Ar * (1 - t);
      } else if (ph >= 0.3 && ph < 0.35) {
        const t = (ph - 0.3) / 0.05;
        yOffset += A * As * (t < 0.5 ? 2 * t : 2 * (1 - t));
      } else if (ph >= 0.66 && ph < 0.9) {
  ...
  function attachListeners() {
    ...
    window.removeEventListener("resize", debouncedBuildSVG);

    window.addEventListener("scroll", update, { passive: true });
    window.addEventListener("resize", debouncedBuildSVG, { passive: true });
  }
